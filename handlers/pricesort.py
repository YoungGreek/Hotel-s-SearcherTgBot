...
def update_data(id, name, hotel_info):
    ...
    with sq.connect('TelegramBot.sqlite3') as con:
        ...
        cur.execute("""INSERT INTO users (name, command, date, time, hotels) VALUES (?, ?, ?, ?, ?)""",
                    (str(id), str(name), str(date), str(time), hotel_info,))
...
@dp.message_handler(Command('lowprice'), state=None)
async def lowprice(message: types.Message, state: FSMContext):
    await message.answer('Топ самых дешёвых отелей в городе\n'
                         'Введите город:')

    await sh_state.city.set()
    await state.update_data(hotels_sort='PRICE')
...
async def highprice(message: types.Message, state: FSMContext):
    await message.answer('Топ отелей, наиболее подходящих по цене и расположению от центра\n'
                         'Введите город:')

    await sh_state.city.set()
    await state.update_data(hotels_sort='BEST_DEAL')
...
async def get_photo(message: types.Message, state: FSMContext):
    ...
    if ans == 'y':
        ...
        if data['hotels_sort'] == 'BEST_DEAL':
            name = '/bestdeal'
        elif data['hotels_sort'] == 'PRICE_HIGHEST_FIRST':
            name = '/highprice'
        elif data['hotels_sort'] == 'PRICE':
            name = '/lowprice'
        else:
            name = ''
        for i in range(data['hotels_amount']):
            ...
            update_data(id, name, hotel_info)
